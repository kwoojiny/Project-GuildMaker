...
public class CloudConnector : MonoBehaviour
{
	...
	private string webServiceUrl = "https://script.google.com/macros/s/AKfycbwTZg2gQbrjDKElm5Bs2ctwW7NE1qt-KCjx4Z6NrHW2hekWPIw/exec";
	private string spreadsheetId = "1TQB-eiiG3mWwJ5GcF59jqXSQGSIMW7kPEOnMejvB0H8"; // If this is a fixed value could also be setup on the webservice to save POST request size.
	private string servicePassword = "testpass";
	private float timeOutLimit = 5f;
    ...
    public static CloudConnector Instance
    {
        get
        {
            if (!_Instance)
            {
                ...
                container.name = "CloudConnector";
                _Instance = container.AddComponent(typeof(CloudConnector)) as CloudConnector;
